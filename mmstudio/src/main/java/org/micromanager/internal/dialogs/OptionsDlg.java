...
public final class OptionsDlg extends MMDialog {
   ...
   public OptionsDlg(CMMCore core, MMStudio mmStudio) {
      ...
      deleteLogCheckBox.setSelected(mmStudio_.getSettingsManager().getShouldDeleteOldCoreLogs());
      deleteLogCheckBox.addActionListener((ActionEvent e) -> {
         mmStudio_.getSettingsManager().setShouldDeleteOldCoreLogs(deleteLogCheckBox.isSelected());
      });

      logDeleteDaysField_ =
         new JTextField(Integer.toString(mmStudio_.getSettingsManager().getCoreLogLifetimeDays()), 2);
      ...
      bufSizeField_ = new JTextField(
            Integer.toString(mmStudio_.getSettingsManager().getCircularBufferSize()), 5);
      ...
      runServer.setSelected(mmStudio.getSettingsManager().getShouldRunZMQServer());
      runServer.addActionListener((ActionEvent arg0) ->  {
         ...
         mmStudio_.getSettingsManager().setShouldRunZMQServer(runServer.isSelected());         
      });
   ...
   private void closeRequested() {
      ...
      mmStudio_.getSettingsManager().setCircularBufferSize(seqBufSize);
      mmStudio_.getSettingsManager().setCoreLogLifetimeDays(deleteLogDays);
